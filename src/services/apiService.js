...
export async function fetchNodePositionData(address) {
  const res = await fetch(
    `${ApiUrl}/historic/node/grabPosition/${encodePathParam(address)}`
  );
  if (!res.ok) {
...
export async function fetchNodeRewardsData(address) {
  const res = await fetch(
    `${ApiUrl}/historic/node/grab-rewards/${encodePathParam(address)}`
  );
  if (!res.ok) {
...
export async function fetchNodeSlashesData(address) {
  const res = await fetch(
    `${ApiUrl}/historic/node/grab-slashes/${encodePathParam(address)}`
  );
  if (!res.ok) {
...
export async function fetchNodeBondData(address) {
  const res = await fetch(
    `${ApiUrl}/historic/node/grab-bond/${encodePathParam(address)}`
  );
  if (!res.ok) {
...
export async function fetchHistoricPerformers(churnCount) {
  const res = await fetch(
    `${ApiUrl}/historic/node/historicPerformers/${encodePathParam(churnCount)}`
  );
  if (!res.ok) {
    throw new Error(
...
export async function fetchChurnsForNode(nodeAddress) {
  const url = `${ApiUrl}/historic/node/grabChurnsForNode/${encodePathParam(
    nodeAddress
  )}`;
...
export async function fetchNetworkChurns() {
  const res = await fetch(`${ApiUrl}/historic/network/grabChurns`);
  if (!res.ok) {
    throw new Error("Failed to fetch network churn history");
  }
  return await res.json();
}
...
export async function fetchBPsForNode(nodeAddress) {
  const res = await fetch(
    `${ApiUrl}/historic/node/grabBPsForNode/${encodePathParam(nodeAddress)}`
  );
  if (!res.ok) {
...
export async function fetchMimirVotes() {
  const res = await fetch(`${ApiUrl}/mimir/votes`);
  if (!res.ok) throw new Error("Failed to fetch Mimir voting data");
  return await res.json();
}

export async function fetchMimirKey(key) {
  const res = await fetch(`${ApiUrl}/mimir/${encodePathParam(key)}`);
